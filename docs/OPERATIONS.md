...
| One page's images never arrive, and the log says "no decoder for this format" | A format Go cannot read. Those go back to Chromium to be decoded ("image decoded by the landside browser" at debug level), so a *persistent* failure means the tab had already closed, or the source is over the 4 MB this path will carry. A capture's `planeside/tabs/<id>/state.json` lists them under `pendingImages`. |
